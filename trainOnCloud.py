...
parser.add_argument(
    ...
    type=str,
    required=True,
    default="./train-b101/b101_code/output",
...
def train():
    ...
    for i in range(1, config["optim"]["Adam_steps"] + 1):
        ...
        if(i % 1000 == 0):
            ms.save_checkpoint(problem, os.path.join(args.train_url, f'model_{config["collision"]}_kn{config["kn"]}.ckpt'))
            saveloss = np.array(loss_list)
            np.save(os.path.join(args.train_url, f'loss_list_of_{config["collision"]}.npy'), saveloss)
        
    ms.save_checkpoint(problem, os.path.join(args.train_url, f'model_{config["collision"]}_kn{config["kn"]}.ckpt'))
    with open(os.path.join(args.train_url, "zhrlog.txt"), "w") as file:
        for item in log:
            file.write(item + '\n')
    ...
    visual(
        ...
        os.path.join(args.train_url, f'Wave_{config["collision"]}_kn{config["kn"]}.png'),
    )
    if config["save_points"] and (config["collision"] != "LR"):
        save_points(
            problem,
            points=1000,
            filename=os.path.join(args.train_url, f'{config["collision"]}_kn{config["kn"]}.npz'),
        )
